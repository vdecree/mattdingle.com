...
function anps_featured_video_add_custom_box() { 
    ...
    foreach($screens as $screen) {
        add_meta_box('anps_featured_video_meta', __('Featured video', ANPS_TEMPLATE_LANG), 'display_meta_box_featured_video', $screen, 'side');
    }
}

function display_meta_box_featured_video( $post ) { 
    $value2 = get_post_meta($post->ID, $key ='anps_featured_video', $single = true ); 
    echo "<input type='text' name='anps_featured_video' value='".$value2."' style='width: 255px'/>";
}
