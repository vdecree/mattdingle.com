...
class WP_Import extends WP_Importer { 
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					...
					printf( __( 'Loading', 'wordpress-importer' ), esc_html( $post['post_author'] ) );
	...
	function get_author_mapping() { 
		...
		$lahko_izpise = true;
		foreach ( (array) $author_name as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ( $lahko_izpise ) {
						echo '<div style="width:100%; text-align:center; margin: 40px 0; font-size: 14px; font-weight: bold; color: #333; font-family: Arial;">' . __( 'Dummy content is being installed, please be patient.', 'wordpress-importer' ) . '</div>';
						$lahko_izpise = false;
					}
				}
...
function wordpress2_importer_init() {
...
add_action( 'admin_init', 'wordpress2_importer_init' );