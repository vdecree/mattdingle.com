...
class Custom_WC_Widget_Top_Rated_Products extends WC_Widget_Top_Rated_Products {
			...
			while ( $r->have_posts() ) {
				...
				echo '<li>
					<a href="' . get_permalink() . '"><div class="image">
						' . ( has_post_thumbnail() ? get_the_post_thumbnail( $r->post->ID, 'product-widget' ) : woocommerce_placeholder_img( 'product-widget' ) ) . ' ' . '
					</div></a><h4><a href="' . get_permalink() . '">' . get_the_title() . '</a></h4><div class="widget-price">' . $product->get_price_html() . '</div><div class="widget-rating">' . $rating . '</div>
				</li>';
			}
