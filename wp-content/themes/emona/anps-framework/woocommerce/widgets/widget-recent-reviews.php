	 ...
	 public function widget( $args, $instance ) {
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment ) {
				...
				echo '<a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '"><div class="image">
						' . ( has_post_thumbnail() ? get_the_post_thumbnail( $comment->comment_post_ID, 'product-widget' ) : woocommerce_placeholder_img( 'product-widget' ) ) . ' ' . '
					</div></a>';

				echo '<h4><a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '">' . $_product->get_title() . '</a></h4>';
