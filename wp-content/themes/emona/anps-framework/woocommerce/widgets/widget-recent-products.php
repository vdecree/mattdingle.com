...
class Custom_WC_Widget_Recent_Products extends WC_Widget_Recent_Products {
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts()) {
				...
				?>
					...
					<?php echo '</a><h4><a href="' . get_permalink() . '">' . get_the_title() . '</a></h4><div class="widget-rating">' . $rating . '</div><div class="widget-price">' . $product->get_price_html() . '</div>
				</li>';
			}
