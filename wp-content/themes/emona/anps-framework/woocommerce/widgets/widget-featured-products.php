...
class Custom_WC_Widget_Featured_Products extends WC_Widget_Featured_Products {
...
?>
		...
		<li><a href="<?php echo esc_url( get_permalink( $r->post->ID ) ); ?>" title="<?php echo esc_attr($r->post->post_title ? $r->post->post_title : $r->post->ID); ?>">
			<div class="image">
				<?php echo $product->get_image("product-widget"); ?>
			</div>
		</a><?php if ( $r->post->post_title ) echo '<h4><a href="' . get_permalink() . '">' . get_the_title( $r->post->ID ) . '</a></h4>'; else echo $r->post->ID; ?><div class="widget-price"><?php echo $product->get_price_html(); ?></div><div class="widget-rating"><?php echo woocommerce_template_loop_rating(); ?></div></li>
