...
if ( $customer_orders ) : ?>
	...
	<h3 class="myaccount-heading"><?php echo apply_filters( 'woocommerce_my_account_my_orders_title', __( 'Recent Orders', 'woocommerce' ) ); ?></h3>
	...
	<table class="shop_table my_account_orders">
		...
		<thead>
			<tr>
				...
				<th class="order-actions"><?php _e( 'Details', 'emona' ); ?></th>
			</tr>
		</thead>
					...
					<td class="order-actions">
						<?php
							...
							foreach( $actions as $key => $action ) {
								echo '<a href="' . esc_url( $action['url'] ) . '" class="btn btn-style-1 btn-sm ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
							}
						?>
					</td>
				</tr><?php
...
<?php else: ?>
	...
	<h3 class="myaccount-heading"><?php _e("My Orders", "emona"); ?></h3>

	<p><?php _e("No orders found!", "emona"); ?></p>
